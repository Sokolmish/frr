...
bfd_local_sockets_add(struct bfd_vrf_global *bvrf, const char *addr_str)
{
	...
	struct bfd_data_sockets *ds;

	ds = XCALLOC(MTYPE_BFDD_LOCAL_SOCKETS, sizeof(*ds));
	ds->bvrf = bvrf;
	...
	if (sa.sa_sin.sin_family == AF_INET) {
		ds->is_ipv6 = false;
		ds->bg_shop = bp_udp_shop(bvrf->vrf, &sa.sa_sin);
		ds->bg_mhop = bp_udp_mhop(bvrf->vrf, &sa.sa_sin);
	} else if (sa.sa_sin6.sin6_family == AF_INET6) {
		ds->is_ipv6 = true;
		ds->bg_shop = bp_udp6_shop(bvrf->vrf, &sa.sa_sin6);
		ds->bg_mhop = bp_udp6_mhop(bvrf->vrf, &sa.sa_sin6);
	} else
	...
	listnode_add(bvrf->sockets, ds);
}

static void
bfd_local_sockets_del(void *data)
{
	struct bfd_data_sockets *ds = data;

	/* All sockets should be closed yet. */
	assert(ds->bg_shop == -1);
	assert(ds->bg_mhop == -1);

	XFREE(MTYPE_BFDD_LOCAL_SOCKETS, ds);
}
...
static int bfd_vrf_disable(struct vrf *vrf)
{
	...
	if (bvrf->sockets) {
		for (ALL_LIST_ELEMENTS_RO(bvrf->sockets, node, ds)) {
			...
			if (ds->bg_shop != -1)
				socket_close(&ds->bg_shop);
			if (ds->bg_mhop != -1)
				socket_close(&ds->bg_mhop);
		}
